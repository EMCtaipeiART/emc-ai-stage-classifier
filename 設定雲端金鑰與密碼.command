#!/bin/bash
# 設定 Cloudflare 線上版的 OpenAI 金鑰與團隊密碼（輸入內容不會顯示，也不會存在電腦上）
set -e
...
echo
echo "【1/2】OpenAI API 金鑰：請貼上完整金鑰（不是 sk-...xxxx 縮寫）後按 Enter。"
...
echo
echo "【2/2】團隊密碼：同事登入網站用，請輸入後按 Enter（至少 8 個字）。不想更換請直接按 Enter。"
read -r -s PASS; echo
if [ -n "$PASS" ]; then
